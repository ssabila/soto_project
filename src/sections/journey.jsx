...
export default function JourneySection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
        ...
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            gsap.set(bgRef.current, {
              x: -self.progress * totalMove * PARALLAX_FACTOR,
            })
          },
        },
      })
      ...
      panels.forEach((panel) => {
        ...
        // Rotasi awal bingkai — sedikit goyah agar terasa organik
        if (frame) gsap.set(frame, { rotation: gsap.utils.random(-2, 2) })

        // Parallax nama region (bergerak ke kiri saat di-scroll)
        if (regionName) {
          gsap.to(regionName, {
            x: -80,
            ease: 'none',
        ...
        // Parallax gambar (sedikit bergeser ke kanan)
        if (img) {
          gsap.to(img, {
            xPercent: 8,
            ease: 'none',
        ...
        // Parallax dekorasi (masing-masing bergerak random)
        decors.forEach((dec) => {
          gsap.to(dec, {
        ...
        // Timeline: rating bintang → divider → teks
  ...
  //  Helpers 
  const renderStars = (rating, config) =>
  ...
  const renderTitle = (name) =>
    name.split(' ').map((word, wIdx, arr) => (
      <span
        key={wIdx}
        style={{
          display:     'inline-flex',
          alignItems:  'baseline',
          marginRight: wIdx < arr.length - 1 ? '0.25em' : 0,
        }}
      >
  ...
  return (
    /**
     * Callback ref menggabungkan dua ref ke satu elemen DOM:
     *   - sectionRef.current  → dipakai oleh GSAP context (ScrollTrigger pin)
     *   - transitionRef.current → dipakai oleh useSectionTransition (opsional,
     *     hook sebenarnya mencari fromEl via querySelector, bukan ref ini;
     *     tapi kita set juga agar hook punya pegangan jika perlu).
     *
     * data-section="journey" WAJIB ada agar:
     *   1. ScrollTrigger dari section berikutnya (unity) bisa menemukannya
     *      sebagai "fromEl" lewat querySelector('[data-section="journey"]').
     *   2. Konsistensi penamaan dengan TRANSITIONS array.
     */
    <section
      ref={(el) => {
          ...
          return (
            ...
            >
              ...
              <img
                src={DECORS[i % 4]}
                alt=""
                className="j-decor journey-decor"
                style={{ ...S.decor, top: '12%', left: '8%' }}
              />
              <img
                src={DECORS[(i + 1) % 4]}
                alt=""
                className="j-decor journey-decor"
                style={{ ...S.decor, bottom: '12%', right: '6%' }}
              />
              <img
                src={DECORS[(i + 2) % 4]}
                alt=""
